...
from data_processor import pad_sequences, batch_yield
...
class BiLSTM_CRF(object):
    ...
    def save_test_result(self,label_list, data, filename=None):
        label2tag = {}
        for tag, label in self.tag2label.items():
            label2tag[label] = tag if label != 0 else label
        ...
        for label_, (sent, tag) in zip(label_list, data):
            ...
            model_predict.append(sent_res)
        predict_result_path=os.path.join(self.test_result_path, filename+'.result')
        with open(predict_result_path, "w", encoding='utf-8') as fw:
            line = []
            for sent_result in model_predict:
                for char, tag_ in sent_result:
                    line.append("{} {}\n".format(char, tag_))
                line.append("\n")
            fw.writelines(line)